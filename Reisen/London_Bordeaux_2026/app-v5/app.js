...
function renderConnections(){
  const cards = state.data.connections.verbindungen.map(item => renderConnectionCard(item)).join('');

  qs('#connections').innerHTML = `
    <section class="panel">
      <h2>Verbindungen</h2>
      <p>Planzeiten fuer die Tagesplanung. Die Google-Maps-Route oeffnet direkt den OePNV-Modus und prueft die Verbindung live.</p>
    </section>
    <section class="connection-list">${cards}</section>
  `;
}
  ...
  return `
    <article class="connection-card">
      <div class="connection-head">
        <div>
          <span class="badge muted">${escapeHtml(formatDateShort(item.datum))}</span>
          <h3>${escapeHtml(item.titel || item.nach)}</h3>
        </div>
        ${item.mapsRoute ? `<a class="button-link route-button" target="_blank" rel="noopener" href="${escapeHtml(item.mapsRoute)}">Google Maps ÖPNV-Route</a>` : ''}
      </div>
      <div class="connection-timebar">
        <div><span>Abfahrt</span><strong>${escapeHtml(item.abfahrtGeplant || '-')}</strong></div>
        <div><span>Ankunft</span><strong>${escapeHtml(item.ankunftGeplant || '-')}</strong></div>
        <div><span>Dauer</span><strong>${escapeHtml(item.dauerPlan || item.zeit || '-')}</strong></div>
        <div><span>Puffer</span><strong>${escapeHtml(item.puffer || '-')}</strong></div>
      </div>
      <div class="route-line">
        <strong>${escapeHtml(item.von)}</strong>
        <span>nach</span>
        <strong>${escapeHtml(item.nach)}</strong>
      </div>
      <dl class="route-grid">
        <div><dt>Verkehr</dt><dd>${escapeHtml(item.verkehrsmittel)}</dd></div>
        <div><dt>Linie / Route</dt><dd>${escapeHtml(item.linie)}</dd></div>
        <div><dt>Richtung</dt><dd>${escapeHtml(item.richtung)}</dd></div>
        <div><dt>Umstieg</dt><dd>${escapeHtml(item.umstieg)}</dd></div>
        <div><dt>Ausstieg</dt><dd>${escapeHtml(item.ausstieg)}</dd></div>
        <div><dt>Fussweg</dt><dd>${escapeHtml(item.fussweg)}</dd></div>
      </dl>
      <p class="route-note">${escapeHtml(item.updateHinweis)}</p>
...
function renderConnectionMini(item){
  return `
    <div class="mini-route">
      <strong>${escapeHtml(item.titel || item.nach)}</strong>
      <span>${escapeHtml(item.abfahrtGeplant || '-')} -> ${escapeHtml(item.ankunftGeplant || '-')} · ${escapeHtml(item.verkehrsmittel)}</span>
      <span>${escapeHtml(item.von)} -> ${escapeHtml(item.nach)}</span>
      ${item.mapsRoute ? `<a href="${escapeHtml(item.mapsRoute)}" target="_blank" rel="noopener">Google Maps ÖPNV-Route</a>` : ''}
    </div>
  `;
}
